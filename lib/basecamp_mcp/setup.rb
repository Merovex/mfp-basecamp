...
module BasecampMcp
  class Setup
    ...
    REDIRECT_PORT = 14_839
    REDIRECT_URI = "http://localhost:#{REDIRECT_PORT}/callback".freeze
    ...
    def run
      ...
      params = "type=web_server&client_id=#{client_id}&redirect_uri=#{URI.encode_www_form_component(REDIRECT_URI)}"
      auth_url = "#{AUTHORIZE_URL}?#{params}"
    ...
    def exchange_code(code, client_id, client_secret)
      ...
      raise "Token exchange failed (#{response.status}): #{response.body}" unless response.success?
    ...
    def fetch_accounts(access_token)
      ...
      raise "Failed to fetch accounts (#{response.status}): #{response.body}" unless response.success?
